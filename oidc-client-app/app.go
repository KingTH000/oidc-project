...
func main() {
	...
	marshalUserinfo := func(w http.ResponseWriter, r *http.Request, tokens *oidc.Tokens[*oidc.IDTokenClaims], state string, rp rp.RelyingParty, info *oidc.UserInfo) {
		...
		userInfoData, err := json.Marshal(info)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		//Call the /api/posts endpoint --
		var postsData []byte
		
		// Create a new request to our resource server
		resourceServerURL := "http://localhost:8080/api/posts"
		req, err := http.NewRequestWithContext(r.Context(), "GET", resourceServerURL, nil)
		if err != nil {
			http.Error(w, "Failed to create request to resource server", http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			http.Error(w, "Failed to call resource server", http.StatusInternalServerError)
			return
		}
		...
		if res.StatusCode != http.StatusOK {
			...
			postsData = []byte(fmt.Sprintf("Error from resource server: %s", string(body)))
		} else {
			postsData, err = io.ReadAll(res.Body)
			if err != nil {
				http.Error(w, "Failed to read posts data", http.StatusInternalServerError)
				return
			}
		}

		// -- Display everything in a simple HTML page --
		...
		fmt.Fprintf(w, `
			<html>
				...
				<body style="font-family: sans-serif;">
					...
					<pre style="background-color: #eee; padding: 10px; border-radius: 5px;">%s</pre>
					<h3>User Posts (from /api/posts endpoint):</h3>
					<pre style="background-color: #eee; padding: 10px; border-radius: 5px;">%s</pre>
					<hr>
